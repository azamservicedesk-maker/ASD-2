...
const MGMT_PERMISSIONS: Record<string,string[]> = {
  Analyst:            ["overview","data","faults","recurring"],
  Logistics:          ["overview","data","replacements"],
  "Technical Manager":["overview","data","faults","performance","recurring","ai","card_lookup"],
  Executive:          ["overview","data","faults","performance","replacements","recurring","ai","card_lookup"],
};
...
function LoginPage({ onLogin }: { onLogin: (u: User) => void }) {
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "radial-gradient(circle at 50% 35%, #0B1C41 0%, #030712 100%)",
      display: "flex",
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 28, textAlign: "center" }}>
        {/* Crisp representation of Azam TV premium double-ring oval logo */}
        <div style={{ width: 150, height: 74, marginBottom: 12, display: "flex", justifyContent: "center", alignItems: "center" }}>
          <svg width="100%" height="100%" viewBox="0 0 140 72" fill="none" xmlns="http://www.w3.org/2000/svg">
            <ellipse cx="70" cy="36" rx="58" ry="28" fill="#040914" />
            <ellipse cx="70" cy="36" rx="55" ry="24" fill="none" stroke="#2563EB" strokeWidth="2.5" />
            <ellipse cx="70" cy="36" rx="51" ry="20" fill="none" stroke="#1D4ED8" strokeWidth="1" />
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <input
            ...
            style={{
              ...
              background: "#1E2B47",
              color: "#FFFFFF",
              border: userFocused ? "1.5px solid #2563EB" : "1px solid #2C3E60",
              borderRadius: 8,
              padding: "12px 16px",
              fontSize: 14,
              fontWeight: 500,
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <input
            ...
            style={{
              ...
              background: "#1E2B47",
              color: "#FFFFFF",
              border: pwFocused ? "1.5px solid #2563EB" : "1px solid #2C3E60",
              borderRadius: 8,
              padding: "12px 16px",
              fontSize: 14,
              fontWeight: 500,
      ...
      <div style={{
        ...
        Azam TV Service Desk · Confidential
      </div>
    </div>
  );
}
...
function TechnicianApp({ user, allJobs, users, onSubmitBatch, onLogout, messages, onSendMessage, onMarkRead, regions, onSaveUsers, onDeleteJob }: { user:User; allJobs:Job[]; users:User[]; onSubmitBatch:(jobs:Job[])=>void; onLogout:()=>void; messages:Message[]; onSendMessage:(m:Omit<Message,"id"|"timestamp"|"read">)=>void; onMarkRead:(id:string)=>void; regions:Region[]; onSaveUsers?: (u: User[]) => void; onDeleteJob?: (id: string) => void }) {
  ...
  const isHqUser = user.role === "admin" || !!(user.region && user.region.toUpperCase().startsWith("HQ"));
  const nav = [
    ...
    <div className="app-layout" style={{ fontFamily:"'DM Sans',sans-serif", background:C.bg }}>
      ...
      <div className="app-main">
        ...
        <div className="page-pad" style={{ flex:1, padding:isMobile?12:24 }}>
          {view==="field_job" && <FieldJobView user={user} onSubmitBatch={onSubmitBatch} regions={regions} onMenu={()=>setMobileMenuOpen(true)}/>}
          {view==="form" && <>
...
// ─── FIELD JOBS ANALYTICS & DIRECTORY VIEW ────────────────────────────────────

function FieldJobsAnalyticsView({ allJobs, users }: { allJobs: Job[]; users: User[] }) {
  const isMobile = useIsMobile();
  ...
  const fieldJobs = useMemo(() => {
    return allJobs.filter(j => j.isFieldJob || j.branch === "Field Agent");
  }, [allJobs]);

  const resolveInfo = useCallback((j: Job) => {
    const isOld = j.technicianId?.startsWith("FIELD_");
    return {
      techName: isOld ? "HQ Operator" : j.technicianName,
      region: j.region || "Unknown",
      agentName: isOld ? j.technicianName : (j.agentName || "Field Agent"),
    };
  }, []);

  const filtered = useMemo(() => {
    return fieldJobs.filter(j => {
      const info = resolveInfo(j);
      if (fRegion && info.region !== fRegion) return false;
      if (fAgent && !info.agentName.toLowerCase().includes(fAgent.toLowerCase())) return false;
      if (fTech && !info.techName.toLowerCase().includes(fTech.toLowerCase())) return false;
      if (fFrom && j.date < fFrom) return false;
      if (fTo && j.date > fTo) return false;
      return true;
    });
  }, [fieldJobs, fRegion, fAgent, fTech, fFrom, fTo, resolveInfo]);

  const groupedData = useMemo(() => {
    const m: Record<string, { techName: string; region: string; agentName: string; total: number; ok: number; fail: number; replacements: number; faultMap: Record<string, number> }> = {};
    filtered.forEach(j => {
      const info = resolveInfo(j);
      const key = `${info.techName}_${info.region}_${info.agentName}`;
      if (!m[key]) {
        m[key] = {
      ...
      const g = m[key];
      g.total++;
      if (j.result === "OK") g.ok++; else g.fail++;
      if (j.replacement === "Yes") g.replacements++;
      if (j.faultType) {
        g.faultMap[j.faultType] = (g.faultMap[j.faultType] || 0) + 1;
      }
    });

    return Object.values(m).map(g => {
      const sortedFaults = Object.entries(g.faultMap).sort((a,b) => b[1] - a[1]);
      const topFault = sortedFaults[0] ? `${sortedFaults[0][0]}` : "—";
      const rate = g.total ? Math.round((g.ok / g.total) * 100) : 0;
      return { ...g, topFault, rate };
    }).sort((a, b) => b.total - a.total);
  }, [filtered, resolveInfo]);

  const total = filtered.length;
  const ok = filtered.filter(j => j.result === "OK").length;
  const fail = filtered.filter(j => j.result === "FAIL").length;
  const replacements = filtered.filter(j => j.replacement === "Yes").length;
  const okRate = total ? Math.round((ok / total) * 100) : 0;

  const allFilteredRegions = useMemo(() => {
    return [...new Set(fieldJobs.map(j => j.region).filter(Boolean))].sort();
  }, [fieldJobs]);
  ...
  function doExportCSV() {
    const rows = filtered.map(j => {
      ...
      return {
        ...
        "Region": info.region,
        "Field Agent (AGENT)": info.agentName,
        "Customer Name": j.customerName,
        "Phone": j.phone || "—",
        "Card Number": j.cardNumber,
        "Fault Type": j.faultType,
        "STB Model": j.modelNumber,
        "Result": j.result,
        "Replacement": j.replacement,
        "Reason": j.replacementReason || "—",
        "Notes": j.troubleshootDescription || "—"
      };
    });
    csvExport(rows, "AzamFieldJobs_Export.csv");
  }
  ...
  function doExportPDF() {
    ...
    pdfExport(
      "Field Jobs Service Report",
      `${filtered.length} field job records`,
      [
        { key: "date", label: "Date" },
        ...
        { key: "card", label: "Card / STB #" },
        { key: "fault", label: "Fault Type" },
        { key: "result", label: "Result" },
        { key: "replacement", label: "Rep." }
      ],
  ...
  return (
    <div className="page-pad" style={{ padding: isMobile ? 12 : 20 }}>
      {/* Search & filters */}
      <Card style={{ marginBottom: 16 }}>
        <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 12, fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: .5 }}>🌍 FILTER FIELD JOBS FOR UPCOUNTRY TRIPS</div>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
          <div style={{ flex: 1, minWidth: 150 }}><label style={LBL}>Region</label>
            <Sel value={fRegion} onChange={e => setFRegion(e.target.value)}>
              <option value="">All Regions</option>
              {allFilteredRegions.map(r => <option key={r} value={r}>{r}</option>)}
            </Sel>
          </div>
          <div style={{ flex: 1, minWidth: 150 }}><label style={LBL}>Field Agent (AGENT)</label>
            <Inp value={fAgent} onChange={e => setFAgent(e.target.value)} placeholder="Search agent name..." />
          </div>
          <div style={{ flex: 1, minWidth: 150 }}><label style={LBL}>Technician</label>
            <Inp value={fTech} onChange={e => setFTech(e.target.value)} placeholder="Search technician..." />
          </div>
          <div><label style={LBL}>From</label><Inp type="date" value={fFrom} onChange={e => setFFrom(e.target.value)} style={{ width: "auto" }} /></div>
          <div><label style={LBL}>To</label><Inp type="date" value={fTo} onChange={e => setFTo(e.target.value)} style={{ width: "auto" }} /></div>
          {(fRegion || fAgent || fTech || fFrom || fTo) && (
            <Btn onClick={() => { setFRegion(""); setFAgent(""); setFTech(""); setFFrom(""); setFTo(""); }} variant="ghost" size="sm">Clear Filters</Btn>
          )}
        </div>
      </Card>
      ...
      <div className="metrics-row" style={{ marginBottom: 20 }}>
        <MetricCard label="Total Field Jobs" value={total.toLocaleString()} icon="🌍" sub="Active field records" />
        <MetricCard label="OK Results" value={ok.toLocaleString()} color={C.success} icon="✅" sub={`${okRate}% success rate`} />
        <MetricCard label="Failed Results" value={fail.toLocaleString()} color={C.red} icon="❌" sub={`${total ? Math.round((fail / total) * 100) : 0}% fail rate`} />
        <MetricCard label="Replacements" value={replacements.toLocaleString()} color={C.warning} icon="🔄" sub={`${total ? Math.round((replacements / total) * 100) : 0}% replacement rate`} />
      </div>

      {/* Grouped Breakdown: Tech + Region + Agent */}
      <Card style={{ marginBottom: 20, padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <span style={{ fontWeight: 700, fontSize: 15, fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: .5 }}>👥 FIELD JOBS PERFORMANCE BY CONTRACT & TARGET</span>
            <div style={{ color: C.muted, fontSize: 11, fontStyle: "italic", marginTop: 2 }}>Grouped by Technician, Region, and Field Agent</div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <Btn onClick={doExportPDF} variant="pdf" size="sm">📄 Export PDF</Btn>
            <Btn onClick={doExportCSV} variant="ghost" size="sm">⬇ Download CSV</Btn>
          </div>
        </div>
        <div className="table-wrap">
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 750 }}>
            <thead>
              <tr style={{ background: "#F5F8FF" }}>
                {["Technician", "Region", "Field Agent (AGENT)", "Total Jobs", "OK", "FAIL", "Replacements", "Success Rate", "Top Fault"].map(h =>
                  <th key={h} style={{ padding: "10px 14px", color: C.muted, textAlign: "left", fontWeight: 700, fontSize: 11, textTransform: "uppercase", letterSpacing: .7, borderBottom: `1px solid ${C.border}`, whiteSpace: "nowrap" }}>{h}</th>
                )}
              </tr>
            </thead>
            <tbody>
              {groupedData.length === 0 ? (
                <tr><td colSpan={9} style={{ padding: 32, textAlign: "center", color: C.muted }}>No field jobs match the selected filter query.</td></tr>
              ) : (
                groupedData.map((g, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${C.border}` }}>
                    <td style={{ padding: "11px 14px", fontWeight: 700 }}>{g.techName}</td>
                    <td style={{ padding: "11px 14px", color: C.muted, fontSize: 12 }}>{g.region}</td>
                    <td style={{ padding: "11px 14px", fontWeight: 600, color: C.blueMid }}>🏷️ {g.agentName}</td>
                    <td style={{ padding: "11px 14px", fontWeight: 700 }}>{g.total}</td>
                    <td style={{ padding: "11px 14px", fontWeight: 700, color: C.success }}>{g.ok}</td>
                    <td style={{ padding: "11px 14px", fontWeight: 700, color: C.red }}>{g.fail}</td>
                    <td style={{ padding: "11px 14px", color: C.warning }}>{g.replacements}</td>
                    <td style={{ padding: "11px 14px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <div style={{ flex: 1, height: 7, background: "#EEF2FF", borderRadius: 4, minWidth: 60 }}>
                          <div style={{ width: `${g.rate}%`, height: "100%", borderRadius: 4, background: g.rate >= 70 ? C.success : g.rate >= 50 ? C.warning : C.red }} />
                        </div>
                        <strong style={{ fontSize: 12, color: g.rate >= 70 ? C.success : g.rate >= 50 ? C.warning : C.red }}>{g.rate}%</strong>
                      </div>
                    </td>
                    <td style={{ padding: "11px 14px", color: C.muted, fontSize: 12 }}>{g.topFault}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      {/* Detailed Field Jobs Table */}
      <Card style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.border}`, fontWeight: 700, fontSize: 15, fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: .5 }}>📋 DETAILED FIELD JOB RECORDS (LATEST FIRST)</div>
        <div className="table-wrap">
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 800 }}>
            <thead>
              <tr style={{ background: "#F9FAFB" }}>
                {["Date", "Technician", "Region", "Field Agent", "Customer Name", "Card / STB #", "Fault Type", "Result", "Replacement"].map(h =>
                  <th key={h} style={{ padding: "10px 14px", color: C.muted, textAlign: "left", fontWeight: 700, fontSize: 11, textTransform: "uppercase", letterSpacing: .7, borderBottom: `1px solid ${C.border}`, whiteSpace: "nowrap" }}>{h}</th>
                )}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={9} style={{ padding: 32, textAlign: "center", color: C.muted }}>No individual field job records match.</td></tr>
              ) : (
                [...filtered].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 150).map(j => {
                  const info = resolveInfo(j);
                  return (
                    <tr key={j.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                      <td style={{ padding: "10px 14px", color: C.muted, fontSize: 12, whiteSpace: "nowrap" }}>{fmtDate(j.date)}</td>
                      <td style={{ padding: "10px 14px", fontWeight: 600 }}>{info.techName}</td>
                      <td style={{ padding: "10px 14px", fontSize: 12 }}>{info.region}</td>
                      <td style={{ padding: "10px 14px", fontSize: 12, color: C.blueMid, fontWeight: 500 }}>{info.agentName}</td>
                      <td style={{ padding: "10px 14px" }}>{j.customerName}</td>
                      <td style={{ padding: "10px 14px", fontFamily: "monospace", fontWeight: 700, fontSize: 12, color: C.text }}>{j.cardNumber}</td>
                      <td style={{ padding: "10px 14px", fontSize: 12 }}>{j.faultType}</td>
                      <td style={{ padding: "10px 14px" }}><Badge color={j.result === "OK" ? "green" : "red"}>{j.result}</Badge></td>
                      <td style={{ padding: "10px 14px" }}><Badge color={j.replacement === "Yes" ? "yellow" : "gray"}>{j.replacement || "No"}</Badge></td>
                    </tr>
                  );
                })
              )}
...
function FieldJobView({ user, onSubmitBatch, regions, onMenu, hideHeader }: { user:User; onSubmitBatch:(jobs:Job[])=>void; regions:Region[]; onMenu?: () => void; hideHeader?: boolean }) {
  const isMobile = useIsMobile();
  const [ctxRegion, setCtxRegion] = useState("");
  const [ctxAgent, setCtxAgent] = useState("");
  const [activeContext, setActiveContext] = useState<{ region: string; agentName: string } | null>(null);
  ...
  const allRegionNames = useMemo(() => {
    const list = [...new Set(regions.map(r => r.name))];
    if (list.length === 0) {
      return ["Dar es Salaam", "Kilimanjaro", "Mwanza", "Arusha", "Dodoma", "Zanzibar", "Tanga", "Mbeya", "Morogoro", "Iringa"];
    }
    return list;
  }, [regions]);
  ...
  async function handleSubmitJobs() {
    ...
    const ok = await confirm(
      `Submit ${valid.length} field job(s) for ${fmtDate(date)} under Agent ${activeContext.agentName} in ${activeContext.region}?`,
      { title:"Confirm Field Job Submission", confirmLabel:"Submit", confirmVariant:"primary" }
    );
    if (!ok) return;
    setSaving(true);
    try {
      const jobs: Job[] = valid.map(r => ({
        ...
        troubleshootDescription:r.troubleshootDescription,
        result:r.result,
        replacement:r.replacement,
        replacementReason:r.replacement==="Yes" ? (r.replacementReason==="Other" ? r.replacementOtherReason : r.replacementReason) : "",
        isFieldJob:true,
        agentName:activeContext.agentName,
      }));
      onSubmitBatch(jobs);
      logActivity(user, "SUBMIT_FIELD_JOBS", `${jobs.length} field job(s) for Agent ${activeContext.agentName} in ${activeContext.region}`);
      setRows([newJobRow()]);
      setSubmitted(true);
      showToast(`${jobs.length} field job(s) submitted successfully!`, "success");
    } catch(e) {
      showToast("Submission failed. Please try again.", "error");
      console.error(e);
    } finally {
  ...
  return (
    <>
      ...
      {!hideHeader && (
        <PageHeader
          title="🌍 Field Job Entry"
          sub={activeContext ? `Logging for ${activeContext.agentName} · ${activeContext.region}` : "HQ Field Job Operations Desk"}
          onMenu={onMenu}
        />
      )}
      <div className="page-pad" style={{ padding:isMobile?12:20 }}>
        {!activeContext ? (
          <div style={{
            ...
            <form onSubmit={handleSetContext} style={{ width: "100%", maxWidth: 460 }}>
              <Card style={{ padding: "32px 28px", boxShadow: "0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)", border: `1px solid ${C.border}` }}>
                <div style={{ textAlign: "center", marginBottom: 20 }}>
                  <div style={{ fontSize: 40, marginBottom: 12 }}>🌍</div>
                  <h3 style={{ textTransform:"uppercase", letterSpacing:.8, fontSize:16, color:C.text, marginBottom:8, fontWeight:800, fontFamily:"'Barlow Condensed',sans-serif" }}>📍 Field Job Context Configuration</h3>
                  <p style={{ color:C.muted, fontSize:13, lineHeight:1.5, margin: 0 }}>HQ operators can batch enter jobs on behalf of any external field agent. Please configure the target region and agent name before inputting rows.</p>
                </div>
                ...
                <div style={{ marginBottom:24 }}>
                  <label style={{ ...LBL, fontSize:11, fontWeight:700 }}>Field Agent Name <span style={{ color:C.red }}>*</span></label>
                  <Inp value={ctxAgent} onChange={e=>setCtxAgent(e.target.value)} placeholder="e.g. John Doe, Fatma Ali" required />
                </div>

                <Btn type="submit" full style={{ padding: 12, fontSize: 13, fontWeight: "bold" }}>Configure & Proceed →</Btn>
              </Card>
            </form>
          </div>
        ) : (
          <>
            <Card style={{ marginBottom: 16, padding: "12px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
              <div style={{ display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap" }}>
                <div style={{ fontSize: 13 }}>
                  <span style={{ color: C.muted, marginRight: 6 }}>Target Agent:</span>
                  <strong style={{ color: C.text, fontSize: 14 }}>{activeContext.agentName}</strong>
                </div>
                <div style={{ fontSize: 13 }}>
                  <span style={{ color: C.muted, marginRight: 6 }}>Region:</span>
                  <strong style={{ color: C.blue, fontSize: 14 }}>📍 {activeContext.region}</strong>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: C.muted }}>Date:</span>
                  <Inp type="date" value={date} onChange={e=>setDate(e.target.value)} disabled={submitted} style={{ width: "auto", padding: "4px 8px", fontSize: 12 }} />
                </div>
              </div>
              <Btn onClick={handleResetContext} variant="ghost" size="sm">⚙ Change Agent/Region</Btn>
            </Card>

            {submitted ? (
              <Card style={{ textAlign:"center", padding:"48px 24px" }}>
                <div style={{ fontSize:52, marginBottom:16 }}>✅</div>
                <h3 style={{ color:C.success, margin:"0 0 8px", fontSize:22, fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:.5 }}>Jobs Recorded Successfully!</h3>
                <p style={{ color:C.muted, margin:"0 0 6px", fontSize:14 }}>Records for Agent {activeContext.agentName} are locked in the system.</p>
                <div style={{ marginTop:24, display:"flex", gap:12, justifyContent:"center" }}>
                  <Btn onClick={handleNewReport} variant="primary" size="sm">+ Enter More Jobs for Same Agent</Btn>
                  <Btn onClick={handleResetContext} variant="ghost" size="sm">Set Another Agent →</Btn>
                </div>
              </Card>
            ) : (
              <>
                {isMobile ? (
                  <>
                    {rows.map((row, i) => <JobRowCard key={row._id} row={row} onChange={(f,v)=>updateRow(row._id,f,v)} onDelete={()=>setRows(r=>r.filter(x=>x._id!==row._id))} index={i}/>)}
                    <Btn onClick={()=>setRows(r=>[...r,newJobRow()])} variant="ghost" full style={{ marginBottom:16 }}>+ Add Another Row</Btn>
                  </>
                ) : (
                  <Card style={{ padding:0, overflow:"hidden", marginBottom:16 }}>
                    <div className="table-wrap">
                    ...
                    <div style={{ padding:"10px 16px", borderTop:`1px solid ${C.border}`, background:"#FAFCFF", display:"flex", gap:10, alignItems:"center" }}>
                      <Btn onClick={()=>setRows(r=>[...r,newJobRow()])} variant="ghost" size="sm">+ Add Row</Btn>
                      <span style={{ color:C.muted, fontSize:12 }}>{rows.length} row(s) to submit</span>
                    </div>
                  </Card>
                )}
                ...
                <div style={{ display:"flex", justifyContent:"flex-end", alignItems:"center", gap:12 }}>
                  {validCount===0 && <span style={{ color:C.muted, fontSize:12 }}>Fill Customer Name + Card # to enable submit</span>}
                  <Btn onClick={handleSubmitJobs} disabled={saving||validCount===0}>
                    {saving ? "Saving…" : `Submit ${validCount} Field Job(s)`}
                  </Btn>
                </div>
              </>
            )}
...
function CardLookupView({ allJobs, onMenu, hideHeader }: { allJobs:Job[]; onMenu?: () => void; hideHeader?: boolean }) {
  ...
  return (
    <>
      {!hideHeader && <PageHeader title="🔎 Card / STB Lookup" sub="Search full decoder history by card number or customer name" onMenu={onMenu}/>}
...
function ActivityLogView({ onMenu, hideHeader }: { onMenu?: () => void; hideHeader?: boolean }) {
  ...
  return (
    <>
      {!hideHeader && <PageHeader title="📋 Activity Log" sub={`${log.length} total entries`} action={<><Btn onClick={doExportPDF} variant="pdf" size="sm">📄 PDF</Btn><Btn onClick={doExportCSV} variant="ghost" size="sm">⬇ CSV</Btn></>} onMenu={onMenu}/>}
      <div className="page-pad" style={{ padding:20 }}>
        <Card style={{ marginBottom:16 }}>
          <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"flex-end" }}>
...
function UsersView({ users, regions, onSave, currentUser, onMenu, hideHeader }: { users:User[]; regions:Region[]; onSave:(u:User[])=>void; currentUser:User; onMenu?: () => void; hideHeader?: boolean }) {
  ...
  async function save() {
    ...
    if (!form.name?.trim()||!form.username?.trim()||!form.role||!ar) { showToast("Fill all required fields.","error"); return; }
    if (modal==="add" && !form.password) { showToast("Password is required.","error"); return; }
    if (form.role==="management" && !form.managementType) { showToast("Select a management type.","error"); return; }
    if (users.find(u=>u.username.toLowerCase()===form.username!.toLowerCase()&&u.id!==form.id)) { showToast("Username is already taken.","error"); return; }
    setSaving(true);
    try {
      let pw = form.password || "";
      if (modal==="add" && pw) pw = await hashPassword(pw);
      const ud = {...form, region:ar, ...(modal==="add"?{password:pw}:{})} as User;
      delete (ud as any).customRegion;
      onSave(modal==="add" ? [...users,{...ud,id:uid(),createdAt:new Date().toISOString()}] : users.map(u=>u.id===form.id?{...u,...ud}:u));
      logActivity(currentUser, modal==="add"?"ADD_USER":"EDIT_USER", `${modal==="add"?"Added":"Edited"}: ${form.name} (${form.role}) — ${ar}`);
      showToast(modal==="add"?"User created successfully.":"User updated.", "success");
      setModal(null);
  ...
  async function delUser(id: string) {
    ...
    if (t?.role === "admin") { showToast("Cannot delete System Admin.", "error"); return; }
    const ok = await confirm(`Delete user "${t?.name}"? This cannot be undone.`, { title:"Delete User" });
    if (!ok) return;
    onSave(users.filter(u=>u.id!==id));
    logActivity(currentUser,"DELETE_USER",`Deleted: ${t?.name} (${t?.role})`);
  ...
  const rb = (r: string) => ({admin:"red",technician:"blue",management:"green",technical_analyst:"teal",otc_manager:"purple",otc_user:"orange"} as Record<string,string>)[r]||"gray";
  const rd = (u: User) => {
    if (u.role === "admin") return "Admin";
    if (u.role === "otc_manager") return "OTC Manager";
    if (u.role === "otc_user") return "OTC Agent";
    return u.role==="management"?(u.managementType||"Management"):u.role==="technical_analyst"?"Tech Analyst":u.role;
  };
  ...
  return (
    <>
      {ConfirmUI}
      {!hideHeader && <PageHeader title="User Management" sub={`${users.length} users registered`} action={<><Btn onClick={doExportPDF} variant="pdf" size="sm">📄 PDF</Btn><Btn onClick={doExportCSV} variant="ghost" size="sm">⬇ CSV</Btn><Btn onClick={openAdd}>+ New User</Btn></>} onMenu={onMenu}/>}
      <div className="page-pad" style={{ padding:20 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:13 }}>
            ...
            <FormField label="Role *">
              <Sel 
                ...
                onChange={e=>setForm({...form,role:e.target.value,managementType:""})}
                disabled={form.role === "admin" && modal === "edit"}
              >
                <option value="technician">Technician</option>
                <option value="management">Management</option>
                <option value="technical_analyst">Technical Analyst</option>
                {form.role === "admin" && <option value="admin">System Admin</option>}
                <option value="otc_manager">OTC Manager</option>
                <option value="otc_user">OTC Agent (User)</option>
              </Sel>
            </FormField>
...
function DataView({ allJobs, users, onMenu, hideHeader, canDownload = false }: { allJobs:Job[]; users:User[]; onMenu?: () => void; hideHeader?: boolean; canDownload?: boolean }) {
  ...
  return (
    <>
      {!hideHeader && <PageHeader title="All Service Records" sub={`${filtered.length} of ${allJobs.length} records`} action={canDownload ? <><Btn onClick={doPDF} variant="pdf" size="sm">📄 PDF</Btn><Btn onClick={doCSV} variant="ghost" size="sm">⬇ CSV</Btn></> : undefined} onMenu={onMenu}/>}
      <div className="page-pad" style={{ padding:20 }}>
...
function UserExportView({ users, onMenu, hideHeader }: { users:User[]; onMenu?: () => void; hideHeader?: boolean }) {
  const [roleF,   setRoleF]   = useState("");
  ...
  return (
    <>
      {!hideHeader && <PageHeader title="👥 Staff Directory Export" sub={`${shown.length} staff members`} action={<><Btn onClick={doPDF} variant="pdf" size="sm">📄 PDF</Btn><Btn onClick={doCSV} variant="ghost" size="sm">⬇ CSV</Btn></>} onMenu={onMenu}/>}
      <div className="page-pad" style={{ padding:20 }}>
...
function AiInsightsPanel({ allJobs, users, onMenu, hideHeader }: { allJobs:Job[]; users:User[]; onMenu?: () => void; hideHeader?: boolean }) {
  const [loading,    setLoading]    = useState(false);
  ...
  return (
    <>
      ...
      {!hideHeader && <PageHeader title="✨ AI Smart Insights" sub={`Powered by ChatGPT · ${allJobs.length} records`}
        action={
...
function MessagesView({ currentUser, users, messages, onSend, onMarkRead, onMenu, hideHeader }: {
  currentUser:User; users:User[]; messages:Message[];
  onSend:(m:Omit<Message,"id"|"timestamp"|"read">)=>void; onMarkRead:(id:string)=>void;
  onMenu?: () => void; hideHeader?: boolean;
}) {
  ...
  return (
    <>
      {!hideHeader && <PageHeader title="💬 Messages" sub={unread>0?`${unread} unread message${unread>1?"s":""}`:"Internal Communications"}
        action={canSend?<Btn onClick={()=>setComposing(true)} variant="primary" size="sm">✏ Compose</Btn>:undefined} onMenu={onMenu}/>}
      <div className="page-pad" style={{flex:1,padding:isMobile?12:24}}>
        {composing&&(
...
function AdminApp({ user, users, regions, allJobs, onSaveUsers, onSaveRegions, onLogout, messages, onSendMessage, onMarkRead, onSubmitBatch }: { user:User; users:User[]; regions:Region[]; allJobs:Job[]; onSaveUsers:(u:User[])=>void; onSaveRegions:(r:Region[])=>void; onLogout:()=>void; messages:Message[]; onSendMessage:(m:Omit<Message,"id"|"timestamp"|"read">)=>void; onMarkRead:(id:string)=>void; onSubmitBatch:(jobs:Job[])=>void }) {
  const [view, setView] = useState("users");
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const unreadMsgsAdmin = useMemo(()=>messages.filter(m=>(m.toId===user.id||m.toId==="all")&&!m.read).length,[messages,user.id]);
  const isHqUser = user.role === "admin" || !!(user.region && user.region.toUpperCase().startsWith("HQ"));

  const regularJobs = useMemo(() => allJobs.filter(j => !j.isFieldJob && j.branch !== "Field Agent"), [allJobs]);
  ...
  return (
    <div className="app-layout" style={{ fontFamily:"'DM Sans',sans-serif", background:C.bg }}>
      <Sidebar user={user} nav={nav} active={view} setActive={setView} onLogout={onLogout} mobileOpen={mobileMenuOpen} onMobileClose={()=>setMobileMenuOpen(false)}/>
      <div className="app-main">
        {view==="users"       && <UsersView users={users} regions={regions} onSave={onSaveUsers} currentUser={user} onMenu={()=>setMobileMenuOpen(true)}/>}
        ...
        {view==="ai"          && <AiInsightsPanel allJobs={regularJobs} users={users} onMenu={()=>setMobileMenuOpen(true)}/>}
        {view==="field_jobs_analytics" && (
          <>
            <PageHeader title="🌍 Field Jobs Desk" sub={`${allJobs.filter(j => j.isFieldJob || j.branch === "Field Agent").length} field jobs recorded`} onMenu={()=>setMobileMenuOpen(true)}/>
            <FieldJobsAnalyticsView allJobs={allJobs} users={users}/>
          </>
        )}
        {view==="messages"    && <MessagesView currentUser={user} users={users} messages={messages} onSend={onSendMessage} onMarkRead={onMarkRead} onMenu={()=>setMobileMenuOpen(true)}/>}
        {view==="user_profile" && <UserProfileView user={user} users={users} onSaveUsers={onSaveUsers} onLogout={onLogout} allJobs={allJobs} onMenu={()=>setMobileMenuOpen(true)}/>}
...
function ManagementApp({ user, users, allJobs, onLogout, messages, onSendMessage, onMarkRead, onSubmitBatch, regions, onSaveUsers }: { user:User; users:User[]; allJobs:Job[]; onLogout:()=>void; messages:Message[]; onSendMessage:(m:Omit<Message,"id"|"timestamp"|"read">)=>void; onMarkRead:(id:string)=>void; onSubmitBatch:(jobs:Job[])=>void; regions:Region[]; onSaveUsers?: (u: User[]) => void }) {
  ...
  const nav = [
    ...
    has("data")         && {key:"data",         icon:"📋", label:"Repair Data"},
  ...
  const regularJobs = useMemo(() => allJobs.filter(j => !j.isFieldJob && j.branch !== "Field Agent"), [allJobs]);
  const techs      = users.filter(u=>u.role==="technician");
  const allRegions = useMemo(()=>[...new Set(regularJobs.map(j=>j.region).filter(Boolean))].sort(),[regularJobs]);
  const allBranches= useMemo(()=>[...new Set(regularJobs.filter(j=>!fRegion||j.region===fRegion).map(j=>j.branch).filter(Boolean))].sort(),[regularJobs,fRegion]);
  const filtered   = useMemo(()=>regularJobs.filter(j=>{if(fRegion&&j.region!==fRegion)return false;if(fBranch&&j.branch!==fBranch)return false;if(fTech&&j.technicianId!==fTech)return false;if(fFrom&&j.date<fFrom)return false;if(fTo&&j.date>fTo)return false;return true;}),[regularJobs,fRegion,fBranch,fTech,fFrom,fTo]);
    ...
    <div className="app-layout" style={{ fontFamily:"'DM Sans',sans-serif", background:C.bg }}>
      ...
      <div className="app-main">
        ...
        <div className="page-pad" style={{ flex:1, padding:isMobile?12:24 }}>
          ...
          {view==="data" && <DataView allJobs={filtered} users={users} onMenu={()=>setMobileMenuOpen(true)} canDownload={user.role==="technical_analyst"}/>}
          {view==="card_lookup" && <CardLookupView allJobs={allJobs} hideHeader={true}/>}
          {view==="faults" && (
            <>{filterBar}
...
function TechnicalAnalystApp({ user, users, allJobs, onLogout, messages, onSendMessage, onMarkRead, onSubmitBatch, regions, onSaveUsers }: { user:User; users:User[]; allJobs:Job[]; onLogout:()=>void; messages:Message[]; onSendMessage:(m:Omit<Message,"id"|"timestamp"|"read">)=>void; onMarkRead:(id:string)=>void; onSubmitBatch:(jobs:Job[])=>void; regions:Region[]; onSaveUsers?: (u: User[]) => void }) {
  ...
  const regularJobs = useMemo(() => allJobs.filter(j => !j.isFieldJob && j.branch !== "Field Agent"), [allJobs]);
  const jobRegions  = useMemo(()=>[...new Set(regularJobs.map(j=>j.region).filter(Boolean))].sort(),[regularJobs]);
  const jobsFiltered = useMemo(()=>regularJobs.filter(j=>(!jobFrom||j.date>=jobFrom)&&(!jobTo||j.date<=jobTo)&&(!jobRegion||j.region===jobRegion)),[regularJobs,jobFrom,jobTo,jobRegion]);
  const isHqUser = user.role === "admin" || !!(user.region && user.region.toUpperCase().startsWith("HQ"));
  const nav = [
    {key:"analytics",  icon:"📈", label:"Analytics"},
  ...
  return (
    <div className="app-layout" style={{ fontFamily:"'DM Sans',sans-serif", background:C.bg }}>
      <Sidebar user={user} nav={nav} active={view} setActive={setView} onLogout={onLogout} mobileOpen={mobileMenuOpen} onMobileClose={()=>setMobileMenuOpen(false)}/>
      <div className="app-main">
        <PageHeader title={nav.find(n=>n.key===view)?.label||"Technical Analysis"} sub={`Technical Analyst · ${regularJobs.length} total records`} onMenu={()=>setMobileMenuOpen(true)}/>
        <div className="page-pad" style={{ flex:1, padding:isMobile?12:24 }}>
          {view==="analytics" && <>
            <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginBottom:16}}>
              <Btn onClick={()=>analyticsExportPDF(regularJobs,users)} variant="pdf" size="sm">📄 Analytics PDF</Btn>
              <Btn onClick={()=>analyticsExportCSV(regularJobs)} variant="ghost" size="sm">⬇ Analytics CSV</Btn>
            </div>
            <AnalyticsView allJobs={regularJobs} users={users}/>
          </>}
          {view==="jobs" && <>
            <Card style={{marginBottom:16}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
              ...
              <span style={{color:C.muted,fontSize:12,alignSelf:"center"}}>{jobsFiltered.length} of {regularJobs.length} records</span>
            </div>
            <Card style={{padding:0,overflow:"hidden"}}>
              <div style={{padding:"12px 18px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          {view==="card_lookup" && <CardLookupView allJobs={allJobs} hideHeader={true}/>}
          {view==="recurring"   && <RecurringContent allJobs={regularJobs}/>}
...
function LoadingScreen() {
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #0A1628 0%, #0F2055 100%)", fontFamily: "sans-serif" }}>
...
export default function App() {
  ...
  <>
    <ToastContainer />
    <LoginPage onLogin={u => { saveSession(u.id); setCurrentUser(u); }} />
  </>
);
  ...
  const logout = () => {
    logActivity(freshUser, "LOGOUT", "Signed out");
    clearSession();
    setCurrentUser(null);
  };
  const msgProps = { messages, onSendMessage:sendMessage, onMarkRead:markRead };
  const base = { user:freshUser, users, regions, allJobs, onLogout:logout };
      ...
      {!["admin","technician","management","technical_analyst","otc_manager","otc_user"].includes(freshUser.role) && (
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", minHeight:"100vh", background:C.bg }}>
          <Card style={{ textAlign:"center", padding:32 }}>
            ...
            <div style={{ fontWeight:700, marginBottom:8 }}>Unknown Role: {freshUser.role}</div>
