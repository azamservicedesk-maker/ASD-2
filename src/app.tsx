...
function FieldJobView({ user, onSubmitBatch, regions, onMenu, hideHeader }: { user:User; onSubmitBatch:(jobs:Job[])=>void; regions:Region[]; onMenu?: () => void; hideHeader?: boolean }) {
  ...
  async function handleSubmitJobs() {
    ...
    try {
      const jobs: Job[] = valid.map(r => ({
        id:uid(),
        technicianId:"FIELD_" + uid(),
        technicianName:activeContext.agentName,
  ...
  return (
    <div className="app-layout" style={{ fontFamily:"'DM Sans',sans-serif", background:C.bg }}>
      ...
      <div className="app-main">
        ...
        {view==="data"        && <DataView allJobs={allJobs} users={users} onMenu={()=>setMobileMenuOpen(true)} canDownload={user.role==="technical_analyst"}/>}
        {view==="analytics"   && <><PageHeader title="Analytics & Performance" sub={`${allJobs.length} total records`} action={user.role==="technical_analyst" ? <Btn onClick={doExportAll} variant="ghost" size="sm">⬇ Export All</Btn> : undefined} onMenu={()=>setMobileMenuOpen(true)}/><AnalyticsView allJobs={allJobs} users={users}/></>}
        ...
        {view==="ai"          && <AiInsightsPanel allJobs={allJobs} users={users} onMenu={()=>setMobileMenuOpen(true)}/>}
        {view==="field_job"   && <FieldJobView user={user} onSubmitBatch={onSubmitBatch} regions={regions} onMenu={()=>setMobileMenuOpen(true)}/>}
        {view==="messages"    && <MessagesView currentUser={user} users={users} messages={messages} onSend={onSendMessage} onMarkRead={onMarkRead} onMenu={()=>setMobileMenuOpen(true)}/>}
      </div>
    </div>
    ...
    <div className="app-layout" style={{ fontFamily:"'DM Sans',sans-serif", background:C.bg }}>
      ...
      <div className="app-main">
        ...
        <div className="page-pad" style={{ flex:1, padding:isMobile?12:24 }}>
          ...
          {view==="ai"          && <AiInsightsPanel allJobs={allJobs} users={users} hideHeader={true}/>}
          {view==="field_job"   && <FieldJobView user={user} onSubmitBatch={onSubmitBatch} regions={regions} onMenu={()=>setMobileMenuOpen(true)}/>}
          {view==="messages"    && <MessagesView currentUser={user} users={users} messages={messages} onSend={onSendMessage} onMarkRead={onMarkRead} hideHeader={true}/>}
          {view==="faults" && (
            <>
